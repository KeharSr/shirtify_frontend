...
//       {/* Added pt-20 class for navbar spacing */}
//       <main className="container mx-auto px-4 sm:px-6 lg:px-8 py-12 pt-20">
//         <motion.div
...
//                     <button
//                       onClick={() =>
//                         removeFromFavorites(favorite._id, favorite.product._id)
//                       }
...
const Favorites = () => {
  ...
  const fetchFavorites = async () => {
    ...
    try {
      const res = await getFavouritesApi();
      if (res.status === 200) {
        // Filter out favorites with null products
              ...
              {favorites.map((favorite) => (
                ...
                >
                  <div className="relative h-64 w-full">
                    ...
                    <div className="w-full h-64 relative">
                      <img
                        src={`http://localhost:5000/products/${getFirstImage(favorite.product)}`}
                        className="w-full h-full object-contain"
                        alt={favorite.product.productName}
                      />
                        ...
                        <div className="flex items-center gap-1">
                          {favorite.product.productSize && (
                            <span className="px-2 py-1 bg-gray-100 rounded-full text-xs text-gray-600">
                              {favorite.product.productSize.length} {favorite.product.productSize.length === 1 ? 'size' : 'sizes'}
                            </span>
                          )}
                          {favorite.product.productColor && (
                            <span className="px-2 py-1 bg-gray-100 rounded-full text-xs text-gray-600">
                              {favorite.product.productColor.length} {favorite.product.productColor.length === 1 ? 'color' : 'colors'}
                            </span>
                          )}
                        </div>
                      </div>
